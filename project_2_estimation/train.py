...
def create_data_loader(data, variable_index, leadtime=0, batch_size=64, shuffle=True, device='cpu'):
    if leadtime==0:
        input_data = data[:, :528]
        target_data = data[:, variable_index]
    else:
        input_data = data[:-leadtime, :528]
...
def plot_losses(train_losses, val_losses, root_path, number, leadtime):
    ...
    plt.grid(True)
    plt.savefig(os.path.join(root_path, f'model/loss_plot_n{number}_l{leadtime}.png'))
    plt.show()
...
def main():
    ...
    ### Leadtime 0 is estimation and Leadtime>0 is forecasting
    number = 9
    leadtime = 0
    ...
    train_loader = create_data_loader(train_set, 545, batch_size=64, shuffle=True, device=device)
    val_loader = create_data_loader(valid_set, 545, batch_size=64, shuffle=False, device=device)
    ...
    plot_losses(train_losses, val_losses, leadtime, root_path, number, leadtime)
